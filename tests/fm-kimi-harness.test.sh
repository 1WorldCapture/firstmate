...
#   pane send-text  a *' --auto' line is the kimi launch (logged to
#                   $FM_FAKE_LAUNCH_LOG, state -> launched); any other text is
#                   a pane-plane pointer attempt (logged to
#                   $FM_FAKE_HERDR_PANE_POINTER_LOG, state -> pointer-typed),
#                   which the agent-plane path must never produce;
  ...
  "pane send-text")
    case "${4:-}" in
      *' --auto')
        ...
        ;;
      *)
        printf '%s\n' "$4" >> "$FM_FAKE_HERDR_PANE_POINTER_LOG"
        printf 'pointer-typed\n' > "$FM_FAKE_KIMI_STATE"
        ;;
    esac
    ;;
...
test_kimi_herdr_stalled_prompt_fails_loudly() {
  ...
  assert_grep 'failed: herdr agent prompt did not accept the kimi brief pointer' "$HOME_DIR/state/$id.status" \
...
test_kimi_herdr_unreached_agent_idle_fails_before_delivery() {
  ...
  assert_grep 'failed: kimi did not reach an idle herdr agent state' "$HOME_DIR/state/$id.status" \
