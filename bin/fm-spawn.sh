...
# Kimi 0.42.0 through 2.0.0 gates a fresh worktree on an interactive
# folder-trust dialog. Every kimi launch pre-registers the directory the pane
# starts in through bin/fm-kimi-trust.sh - a failed registration refuses the
# spawn - so the dialog should never appear. The launch-readiness loop still
# reads the visible viewport - so the spawn refuses at preflight on a backend
# with no viewport-bounded capture - and recognizes the complete dialog if one
# appears anyway: it re-selects the already highlighted affirmative option on
# every poll the complete dialog is still there, refuses any ready verdict
# while dialog text is on that pane, and requires two consecutive captures
# that are each ready and dialog-free before the ordinary readiness gates can
# pass. A blank viewport read proves nothing either way: it costs the poll and
# restarts that count. A viewport read that fails outright fails readiness at
# once.
